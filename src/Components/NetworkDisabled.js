            ...
            return (
                <View style={[styles.container,CommonUtil.isEmpty(this.props.isFull) ? {height:SCREEN_HEIGHT*0.9} : {height : SCREEN_HEIGHT}]}>
                    <View style={styles.outerWrap}>
                        <Image source={require('../../assets/icons/icon_network.png')} resizeMode='contain' style={styles.imageWrap} />
                        <View style={styles.textWrap}>
                            <Text stlye={styles.text}>네트워크에 접속할 수 없습니다.</Text>
                            <Text style={styles.networkSubText}>네트워크 연결 상태를 확인해 주세요</Text>
                        </View>
                        <View>
                            <Button title=' 재시도 ' style={{borderColor:'#ccc'}}onPress= {()=> this.checkNetwork()}/>
                        </View>
                    </View>
                </View>
            )
...
const styles = StyleSheet.create({
    ...
    outerWrap : {
        flex:1,width:SCREEN_WIDTH,height:SCREEN_HEIGHT,justifyContent: 'center',alignItems: 'center'
    },
    imageWrap : {
        width:CommonUtil.dpToSize(70),height:CommonUtil.dpToSize(50)
    },
    textWrap : {
        marginVertical:20,alignItems:'center',justifyContent:'center'
    },
    text : {
        paddingVertical:20,color:DEFAULT_COLOR.base_color_222,fontSize:PixelRatio.roundToNearestPixel(DEFAULT_TEXT.head_large)
    }
});